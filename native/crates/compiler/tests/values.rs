...
fn a_published_values_entry_answers_with_what_it_names() {
    let harness: &str = r#"
        ...
        extern uint32_t entry(int64_t *) __asm__("PREFIXsouther3.m$value$ys");
...
/// calls `souther3.publisher$value$ys`, the same exported entry `reader` would import from a
...
fn a_behavior_answering_with_another_modules_published_value_runs_it_there() {
    let harness: &str = r#"
        ...
        extern uint32_t g(int64_t *) __asm__("PREFIXsouther3.reader.g");
