...
fn over_answering(op: &str, left: &str, right: &str, answers: &str) -> String {
    ...
    format!(
        r#"{{"transport":25,"declarations":[{{"module":"counting","name":"Amount","by":"amodule","is":"newtype","field":{{"name":"value","binding":0,"codec":{{"is":"scalar","scalar":"INT"}}}},"invariants":[]}}],"behaviors":[],"modules":[{{"name":"calculation","publishes":[],"helpers":[{held}],"values":[],"entries":[],"definitions":[],"examples":[]}}]}}"#
    )
}
...
/// not the behavior: the value itself only passes through, and nothing lowers a `Date` until the
/// entry that has to write one out.
#[test]
fn a_published_answer_with_a_date_field_is_not_lowered_where_it_is_written() {
    let document = concat!(
        r#"{"transport":25,"declarations":["#,
    ...
    let refused = object_for(document).expect_err("no way yet to write a Date out");
...
fn two_targets_written_the_same_are_the_halves_disagreeing() {
    let inner = r#"{"module":"m","name":"inner","is":"body","parameters":{"named":[{"name":"p0","input":{"is":"scalar","scalar":"INT"}}]},"output":{"is":"scalar","scalar":"INT"},"requirements":[],"ensures":{"at":"none"}}"#;
    let document = composed_document().replace(
        inner,
