...
fn over(op: &str, left: &str, right: &str) -> String {
    ...
    format!(
        r#"{{"transport":6,"declarations":[],"behaviors":[{target}],"modules":[{{"name":"calculation","helpers":[],"values":[],"definitions":[{held}],"examples":[]}}]}}"#
    )
}
...
fn a_transport_from_another_version_is_refused() {
    let later = document("ADD", "INT").replace(r#""transport":6"#, r#""transport":7"#);
...
fn composed_document() -> String {
    concat!(
        ...
        r#""modules":[{"name":"m","helpers":[],"values":[],"definitions":["#,
